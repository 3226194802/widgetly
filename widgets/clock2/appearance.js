// 数字时钟外观调节 —— 渲染进程：背景颜色 / 字体颜色（实时生效）
const { ipcRenderer } = require('electron');
const instId = new URLSearchParams(location.search).get('inst') || 'clock2-1';
...
function wire(key, containerId, pickerId, list) {
  ...
  list.forEach((v) => {
    ...
    s.addEventListener('click', () => {
      appearance[key] = v;
      ipcRenderer.send('clock2:appearance:' + instId, { [key]: v });
      refresh();
    });
  ...
  picker.addEventListener('input', (e) => {
    ...
    ipcRenderer.send('clock2:appearance:' + instId, { [key]: e.target.value });
    refresh();
  });
}
...
document.getElementById('closeBtn').addEventListener('click', () => ipcRenderer.send('clock2:appearance-close:' + instId));
document.getElementById('doneBtn').addEventListener('click', () => ipcRenderer.send('clock2:appearance-close:' + instId));

(async () => {
  try {
    const a = await ipcRenderer.invoke('clock2:appearance-init:' + instId);
